...
create table if not exists public.crm_campaigns (
  ...
  name text not null,
  template_id bigint references public.email_templates(id) on delete set null,
  list_id bigint references public.crm_lists(id) on delete set null,
  tag_slug text,
  definition jsonb,
  status text not null default 'draft' check (status in ('draft','scheduled','running','completed','paused','error')),
...
begin
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='crm_campaigns' and policyname='Admins manage crm_campaigns'
  ) then
...
create table if not exists public.crm_campaign_recipients (
  ...
  contact_id uuid not null references public.crm_contacts(id) on delete cascade,
  status text not null default 'pending' check (status in ('pending','queued','sent','error','skipped')),
  error text,
  sent_at timestamptz,
  created_at timestamptz default timezone('utc'::text, now()),
...
begin
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='crm_campaign_recipients' and policyname='Admins manage crm_campaign_recipients'
  ) then
    create policy "Admins manage crm_campaign_recipients"
...
do $$ begin
  perform 1 from pg_trigger where tgname = 'crm_campaigns_set_updated_at';
  if not found then
    create trigger crm_campaigns_set_updated_at before update on public.crm_campaigns for each row execute function public.set_updated_at_now();
  end if;
end $$;
